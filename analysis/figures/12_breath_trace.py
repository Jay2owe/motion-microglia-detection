...
@figure(
    number=12,
    slug="breath-trace",
    summary="cell-footprint area gained, lost and retained between frames",
    title="Cell-footprint area gained, lost and retained between frames",
...
def build(ctx: FigureContext) -> FigureResult:
    ...
    panels = ctx.panels()
    fig, axes = ctx.layout(panels, gap_inches=1.45)
    ...
    if "breath" in axes:
        ...
        surveillance_panels.mirrored_ledger(
            ...
            net_column="net_area", hour_ticks=hour_ticks,
    ...
    if "small_multiples" in axes:
        ...
        for identity in cells:
            ...
            values = group["gained_area"] - group["lost_area"]
            drawn_values.extend(values.to_numpy(float))
            ax.plot(group["hours"], values,
        ...
        ax.set_xticks(ctx.theme.hour_ticks(
            float(data["hours"].min()), float(data["hours"].max()), hour_ticks
        ))
        finite = [value for value in drawn_values if np.isfinite(value)]
        if finite:
            ctx.theme.set_value_ticks(ax, low=min(finite), high=max(finite))
