...
__all__ = ["OPTIONS", "PLACEMENTS", "SWITCHES", "Length", "Vocabulary",
           "as_length", "commas", "exactly", "length", "numbers", "option",
           "raw_flag", "skip_tokens", "switch"]
...
OPTIONS: dict[str, Vocabulary] = {entry.name: entry for entry in (
    ...
    Vocabulary("cell_lut", "colour map for the cell images, e.g. gray, magma, viridis",
               metavar="NAME"),
    ...
    Vocabulary("outline", "colour of the outline drawn over each image tile",
               metavar="COLOUR"),
    Vocabulary("fit",
               "which traces carry the cosinor curve: a metric list, all, or empty for none",
               cast=commas, metavar="COL,COL"),
    ...
    Vocabulary("shuffles", "number of shuffled-time null replicates",
               cast=int, metavar="N"),
    Vocabulary("regime", "regime number used for a within-cell contrast",
               cast=int, metavar="N"),
    Vocabulary("contour", "persistent-core contour as a share of observed frames",
               cast=float, metavar="0-1"),
...
SWITCHES: dict[str, str] = {
    ...
    "overlay": "draw a spatial mask over the raw image instead of alone",
}
