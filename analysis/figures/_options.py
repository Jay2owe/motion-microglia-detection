...
OPTIONS: dict[str, Vocabulary] = {entry.name: entry for entry in (
    ...
    Vocabulary("fit",
               ...
               cast=commas, metavar="COL,COL"),
    Vocabulary("fit_method",
               "Circadian Workbench period method; significance-giving methods can also call rhythmicity",
               metavar="NAME"),
    Vocabulary("period_methods",
               "Circadian Workbench period estimators compared by an audit figure, comma separated",
               cast=commas, metavar="NAME,NAME"),
    Vocabulary("significance_method",
               "rhythmicity test; unset uses the fitter's test, or Lomb-Scargle for fit-only methods",
               metavar="NAME"),
    Vocabulary("secondary_significance_method",
               "optional second rhythmicity test; when set, both tests must be significant",
               metavar="NAME"),
    Vocabulary("period_config",
               "extra Circadian Workbench period settings as a JSON object",
               cast=json.loads, metavar="JSON"),
    Vocabulary("detrend",
               "Circadian Workbench detrending applied before display and period fitting",
               metavar="NAME"),
    Vocabulary("detrend_methods",
               "Circadian Workbench detrending methods compared by an audit figure, comma separated",
               cast=commas, metavar="NAME,NAME"),
    Vocabulary("detrend_window_hours",
               "Circadian Workbench baseline window in hours; unset inherits the main analysis",
               cast=float, metavar="HOURS"),
    Vocabulary("detrend_polynomial_degree", "degree for polynomial baseline removal",
               cast=int, metavar="N"),
    Vocabulary("detrend_min_valid_fraction", "minimum valid fraction in a baseline window",
               cast=float, metavar="FRACTION"),
    Vocabulary("detrend_bandwidth_hours", "kernel baseline bandwidth in hours",
               cast=float, metavar="HOURS"),
    Vocabulary("detrend_low_cut_hours", "longest period retained by frequency detrending",
               cast=float, metavar="HOURS"),
    Vocabulary("detrend_high_cut_hours", "shortest period retained by frequency detrending",
               cast=float, metavar="HOURS"),
    Vocabulary("detrend_filter_order", "order of the frequency detrending filter",
               cast=int, metavar="N"),
    Vocabulary("detrend_lowess_fraction",
               "fraction of finite samples in each LOWESS local fit",
               cast=float, metavar="0-1"),
    Vocabulary("detrend_lowess_iterations",
               "robust reweighting passes after the first LOWESS fit",
               cast=int, metavar="N"),
    Vocabulary("detrend_asls_smoothness",
               "asymmetric least-squares smoothness penalty",
               cast=float, metavar="NUMBER"),
    Vocabulary("detrend_asls_asymmetry",
               "weight below 0.5 assigned above the asymmetric baseline",
               cast=float, metavar="0-0.5"),
    Vocabulary("detrend_asls_iterations",
               "asymmetric least-squares reweighting passes",
               cast=int, metavar="N"),
    Vocabulary("period_min_hours",
               "shortest candidate period in a broad rhythm search, in hours",
               cast=float, metavar="HOURS"),
    Vocabulary("period_max_hours",
               "longest candidate period in a broad rhythm search, in hours",
               cast=float, metavar="HOURS"),
    Vocabulary("rhythmic_alpha",
               "corrected significance threshold used to call a trace rhythmic",
               cast=float, metavar="0-1"),
