...
@figure(
    ...
    panels=(
        ...
        Panel("field", coupling_panels.phase_map,
...
def build(ctx: FigureContext) -> FigureResult:
    ...
    null_level = float(data["phase_difference"].median())
    if "distance" in axes:
        coupling_panels.phase_difference(axes["distance"], data["distance"], data["phase_difference"],
                                         ctx.theme, null_level=null_level,
    ...
    if "field" in axes:
        metric = metrics[0]
        phases = rhythms[rhythms["metric"] == metric][["identity", "cosinor_peak_hour"]]
        ...
        phase_handle = coupling_panels.phase_map(
            axes["field"], mapped[["centroid_x", "centroid_y"]],
            mapped["cosinor_peak_hour"], ctx.theme, field=_scaled_field(ctx),
            cmap=ctx.option("trace_luts"),
        ...
        common.inset_colour_bar(axes["field"], phase_handle, ctx.theme,
                                label="Fitted peak time (h)", ticks=[0, 6, 12, 18, 24])
    if "measures" in axes:
        for metric, group in data.groupby("metric"):
            axes["measures"].scatter(group["distance"], group["phase_difference"],
        ...
        axes["measures"].set_ylabel("Peak time difference (h)")
        common.semantic_legend(axes["measures"], ctx.theme, location="inside")
    figure_data = data[["identity_a", "identity_b", "distance", "metric",
                        "phase_difference", "overlap_frames"]]
    return FigureResult(
        figure=fig,
        axes=list(axes.values()),
        figure_data=figure_data,
        subtitle=f"One row per identity pair; random-pair comparison level {null_level:.2f} h.",
    )
