"""Production adapter for fragmented encounter-lineage calibration."""
...
import fragmented_encounter_lineage_calibration as calibration


CONFIG_KEY = "field_wide_fragmented_encounter_lineage_calibration"
STAGE_NAME = "110_fragmented_encounter_lineage_calibration"
TOP_LEVEL_KEYS = {
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "fragmented encounter-lineage production configuration contains "
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    """Join only event fragments proved to share a durable encounter."""
    enabled, params = _configured(config_values)
    before = labels.copy()
    before_unclaimed = unclaimed.copy()
    if enabled:
        calibrated, calibrated_members, audit, applications, summary = \
            calibration.calibrate(
                events, members, points, int(len(labels)), params)
    else:
        ...
        summary = {
            ...
            "target_counts": {key: 0 for key in (
                ...
                "review_cases")},
            "event_pairs_audited": 0, "eligible_event_pairs": 0,
            "applied_event_joins": 0,
            "before": calibration._event_summary(events),
            "after": calibration._event_summary(events),
        }
    if not np.array_equal(labels, before):
        raise AssertionError(
            "fragmented encounter-lineage calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "fragmented encounter-lineage calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "fragmented_encounter_lineage_audit.csv",
        "applications": (
            stage / "fragmented_encounter_lineage_applications.csv"),
    ...
    metrics = {
        **summary,
        ...
        "applications_sha256": _sha256(paths["applications"]),
        "parameters": params,
    }
    paths["metrics"].write_text(
        json.dumps(metrics, indent=2) + "\n", encoding="utf-8")
    return (before, before_unclaimed, calibrated, calibrated_members,
            audit, applications, metrics)
