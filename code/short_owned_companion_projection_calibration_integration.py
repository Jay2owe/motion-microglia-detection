"""Production adapter for short same-owner companion event calibration."""
...
import short_owned_companion_projection_calibration as calibration


CONFIG_KEY = "field_wide_short_owned_companion_projection_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "short-companion production configuration contains unsupported "
            "keys (selectors are forbidden): " + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError("short-companion production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    """Calibrate current events while preserving biological ledgers exactly."""
    ...
    if enabled:
        ...
        summary = {
            ...
            "events_calibrated_non_disruptive": 0,
            "removed_event_ids": [],
            "before": calibration.event_summary(events),
            "after": calibration.event_summary(events),
        }
    if not np.array_equal(labels, before):
        raise AssertionError("short-companion calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError("short-companion calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "short_owned_companion_audit.csv",
