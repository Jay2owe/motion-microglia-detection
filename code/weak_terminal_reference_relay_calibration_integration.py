"""Production adapter for weak terminal reference-relay calibration."""
...
import weak_terminal_reference_relay_calibration as calibration


CONFIG_KEY = "field_wide_weak_terminal_reference_relay_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "weak terminal relay production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError("weak terminal relay production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, raw: np.ndarray,
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("weak terminal relay calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "weak terminal relay calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "weak_terminal_reference_relay_audit.csv",
