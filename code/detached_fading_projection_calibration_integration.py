"""Production adapter for detached fading-projection score calibration."""
...
import detached_fading_projection_calibration as calibration


CONFIG_KEY = "field_wide_detached_fading_projection_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "detached fading-projection production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "detached fading-projection production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    if enabled:
        ...
        summary = {
            ...
            "simple_unclaimed_events_audited": int(
                events.family.eq("unclaimed_visible_body").sum()),
            "events_removed_as_nonbiological": 0,
            "removed_event_ids": [],
    ...
    if not np.array_equal(labels, before):
        raise AssertionError(
            "detached fading-projection calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "detached fading-projection calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "detached_fading_projection_audit.csv",
