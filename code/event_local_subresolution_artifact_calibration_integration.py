"""Production adapter for event-local subresolution calibration."""
...
import event_local_subresolution_artifact_calibration as calibration


CONFIG_KEY = "field_wide_event_local_subresolution_artifact_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "event-local subresolution production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "event-local subresolution production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, raw: np.ndarray,
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("event-local subresolution changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError("event-local subresolution changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "event_local_subresolution_audit.csv",
