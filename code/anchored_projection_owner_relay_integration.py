"""Production adapter for anchored projection owner relay recovery."""
...
import anchored_projection_owner_relay as relay


CONFIG_KEY = "field_wide_anchored_projection_owner_relay"
STAGE_NAME = "99_anchored_projection_owner_relay"
TOP_LEVEL_KEYS = {
...
def run(labels: np.ndarray, unclaimed: np.ndarray, raw: np.ndarray,
        config_values: dict[str, Any], output_root: Path,
                   ...
                   dict[str, Any]]:
    """Apply complete-field relay discovery to current-run evidence."""
    ...
    if not points_path.is_file():
        raise FileNotFoundError(
            "anchored-projection relay evidence is missing: "
            + str(points_path))
    points = pd.read_csv(points_path)
    if enabled:
        audit, internal, attached = relay.discover(labels, points, params)
        candidate, applications, details = relay.apply(
            labels, raw, attached, internal, params)
    else:
        audit = pd.DataFrame(columns=relay.AUDIT_COLUMNS)
        applications = pd.DataFrame(columns=relay.APPLICATION_COLUMNS)
    ...
    if int(details["new_duplicate_components"]):
        raise AssertionError(
            "anchored-projection relay created unexplained duplicates")

    audit.to_csv(stage / "anchored_projection_owner_relay_audit.csv", index=False)
    applications.to_csv(
        stage / "anchored_projection_owner_relay_applications.csv", index=False)
