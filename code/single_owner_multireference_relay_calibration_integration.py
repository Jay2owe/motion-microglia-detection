"""Production adapter for single-owner multireference-relay calibration."""
...
import single_owner_multireference_relay_calibration as calibration


CONFIG_KEY = "field_wide_single_owner_multireference_relay_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "single-owner multireference production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "single-owner multireference production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    if not np.array_equal(labels, before):
        raise AssertionError(
            "single-owner multireference calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "single-owner multireference calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "single_owner_multireference_relay_audit.csv",
