"""Production adapter for terminal boundary event calibration."""
...
import terminal_boundary_event_calibration as calibration


CONFIG_KEY = "field_wide_terminal_boundary_event_calibration"
STAGE_NAME = "107_terminal_boundary_event_calibration"
TOP_LEVEL_KEYS = {
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "terminal boundary event configuration contains unsupported keys "
            "(selectors are forbidden): " + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError("terminal boundary event calibration must be field-wide")
    params = deepcopy(configured.get("parameters", {}))
    params["targeting_mode"] = "field_wide_discovery"
    calibration.terminal.assert_target_free(params)
...
def run(labels: np.ndarray, unclaimed: np.ndarray, events: pd.DataFrame,
        members: pd.DataFrame, applications: pd.DataFrame,
        frames: pd.DataFrame, config_values: dict[str, Any],
        output_root: Path,
        ) -> tuple[np.ndarray, np.ndarray, pd.DataFrame, pd.DataFrame,
                   pd.DataFrame, dict[str, Any]]:
    """Remove only alerts resolved by the complete producer ledger."""
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("terminal boundary calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError("terminal boundary calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "terminal_boundary_event_calibration_audit.csv",
    ...
    metrics = {
        ...
        "removed_event_ids": list(removed),
        "before": calibration.event_summary(events),
        "after": calibration.event_summary(calibrated),
