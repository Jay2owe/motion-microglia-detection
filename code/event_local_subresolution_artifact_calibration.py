"""Remove event-local, raw-proven subresolution point artifacts.
...
def assert_target_free(params: dict) -> None:
    ...
    if supplied:
        raise ValueError(
            "event-local subresolution calibration received forbidden targets: "
            + ", ".join(supplied))
    if params.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "event-local subresolution calibration must be field-wide")
...
def calibrate(events: pd.DataFrame, members: pd.DataFrame,
    ...
    if missing_points:
        raise ValueError(
            "event-local subresolution calibration requires scored point "
    ...
    if missing_thresholds:
        raise ValueError(
            "event-local subresolution calibration requires threshold fields: "
            + ", ".join(missing_thresholds))
    movie = int(len(raw))
    minimum_event = int(math.ceil(movie * float(
        params.get("minimum_event_span_movie_fraction", 0.15))))
    minimum_track = int(math.ceil(movie * float(
        params.get("minimum_track_span_movie_fraction", 0.30))))
    minimum_visible = float(params.get("minimum_visible_fraction", 0.70))
    ...
    maximum_strong = float(params.get("maximum_event_strong_fraction", 0.0))
    maximum_core = float(params.get(
        "maximum_core_area_fraction_of_typical_soma", 0.04))
    ...
    for event in events.itertuples(index=False):
        ...
        strong_fraction = float(event_rows.strong.map(_truth).mean()) \
            if len(event_rows) else 1.0
        if event_span < minimum_event:
            reasons.append("event_too_short")
        if track_span < minimum_track:
            reasons.append("reference_span_too_short")
        ...
        if strong_fraction > maximum_strong:
            reasons.append("strong_event_observation_present")
        ...
        audits.append({
            ...
            "eligible": eligible,
            "reason": ("event_local_subresolution_artifact" if eligible
                       else "|".join(reasons)),
        })
...
def run(_upstream_dir: Path | None, params: dict, out) -> dict:
    ...
    outputs = {
        ...
        "audit": out.out / "event_local_subresolution_audit.csv",
