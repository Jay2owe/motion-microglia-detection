"""Production adapter for detached projection owner-relay calibration."""
...
import detached_projection_owner_relay_calibration as calibration


CONFIG_KEY = "field_wide_detached_projection_owner_relay_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "detached projection owner-relay production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "detached projection owner-relay production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    if enabled:
        ...
        summary = {
            ...
            "compound_candidates_audited": 0,
            "events_reclassified": 0,
            "sources_excluded_as_projection_relays": 0,
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("detached projection owner relay changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "detached projection owner relay changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "detached_projection_owner_relay_audit.csv",
