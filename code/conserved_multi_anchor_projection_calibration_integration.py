"""Production adapter for conserved multi-anchor score calibration."""
...
import conserved_multi_anchor_projection_calibration as calibration


CONFIG_KEY = "field_wide_conserved_multi_anchor_projection_calibration"
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    if enabled:
        calibrated, calibrated_members, audit, removed = calibration.calibrate(
            labels, events, members, points, params)
    else:
    ...
    if not np.array_equal(labels, before):
        raise AssertionError(
            "conserved multi-anchor calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "conserved multi-anchor calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "conserved_multi_anchor_encounter_audit.csv",
