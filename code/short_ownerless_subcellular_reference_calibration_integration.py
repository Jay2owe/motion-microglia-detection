"""Production adapter for short ownerless subcellular calibration."""
...
import short_ownerless_subcellular_reference_calibration as calibration


CONFIG_KEY = "field_wide_short_ownerless_subcellular_reference_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "short ownerless subcellular production configuration contains "
            "unsupported keys (selectors are forbidden): "
            + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError(
            "short ownerless subcellular production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, raw: np.ndarray,
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("short ownerless subcellular changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError("short ownerless subcellular changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "short_ownerless_subcellular_audit.csv",
