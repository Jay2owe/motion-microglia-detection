"""Production adapter for recurrent same-owner branch-swarm calibration."""
...
import recurrent_same_owner_branch_swarm_calibration as calibration


CONFIG_KEY = "field_wide_recurrent_same_owner_branch_swarm_calibration"
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "branch-swarm production configuration contains unsupported "
            "keys (selectors are forbidden): " + ", ".join(unexpected))
    if configured.get("targeting_mode", "field_wide_discovery") != \
            "field_wide_discovery":
        raise ValueError("branch-swarm production must be field-wide")
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    """Calibrate current events while preserving mask ledgers exactly."""
    ...
    if not np.array_equal(labels, before):
        raise AssertionError("branch-swarm calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError("branch-swarm calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "recurrent_branch_swarm_audit.csv",
