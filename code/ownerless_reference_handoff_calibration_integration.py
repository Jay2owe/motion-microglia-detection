"""Production adapter for ownerless reference-handoff score calibration."""
...
import ownerless_reference_handoff_calibration as calibration


CONFIG_KEY = "field_wide_ownerless_reference_handoff_calibration"
STAGE_NAME = "112_ownerless_reference_handoff_calibration"
TOP_LEVEL_KEYS = {
    "version", "enabled", "enabled_by_default", "score_only",
    "targeting_mode", "reviewed_event_catalogue_sha256",
    "reviewed_event_members_sha256", "parameters",
}
...
def _configured(values: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if unexpected:
        raise ValueError(
            "ownerless reference-handoff production configuration contains "
...
def run(labels: np.ndarray, unclaimed: np.ndarray, points: pd.DataFrame,
    ...
    if enabled:
        calibrated, calibrated_members, audit, summary = \
    ...
    if not np.array_equal(labels, before):
        raise AssertionError(
            "ownerless reference-handoff calibration changed labels")
    if not np.array_equal(unclaimed, before_unclaimed):
        raise AssertionError(
            "ownerless reference-handoff calibration changed unclaimed data")
    ...
    paths = {
        ...
        "audit": stage / "ownerless_reference_handoff_audit.csv",
